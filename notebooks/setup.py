# Setup require to connect Django into jupyter notebooks
...
def init(verbose=False):
    ...
    try:
        ...
        if verbose:
            print("nest_asyncio patch not available, skipping patch")

    os.chdir(DJANGO_PROJECT_ROOT)
    sys.path.insert(0, str(DJANGO_PROJECT_ROOT))
    
    if verbose:
        print(f"Changed project directory to : {DJANGO_PROJECT_ROOT}")
